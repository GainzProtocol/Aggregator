...
import AggreSwap from "../build/AggreSwap.json";
...
export async function v2Fixture(
  ...
  await factoryV1.initializeFactory(
    (await deployContract(wallet, UniswapV1Exchange, [])).address
  );
  ...
  // deploy routers
  const router01 = await deployContract(
    wallet,
    UniswapV2Router01,
  ...
  // deploy migrator
  const migrator = await deployContract(
    wallet,
    UniswapV2Migrator,
    [factoryV1.address, router01.address],
    overrides
  );

  // initialize V1
  await factoryV1.createExchange(WETHPartner.address, overrides);
  const WETHExchangeV1Address = await factoryV1.getExchange(
    WETHPartner.address
  );
  const WETHExchangeV1 = new Contract(
    WETHExchangeV1Address,
    JSON.stringify(UniswapV1Exchange.abi),
    provider
  ).connect(wallet);
  ...
  // deploy AggreSwap
  const aggreSwap = await deployContract(wallet, AggreSwap);
