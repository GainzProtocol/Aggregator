...
describe('UniswapV2Router{01,02}', () => {
  for (const routerVersion of Object.keys(RouterVersion)) {
    ...
    describe(routerVersion, () => {
      ...
      describe('swapExactETHForTokens :: Aggreswap', () => {
        ...
        beforeEach(async () => {
          ...
          // await token0.approve(router.address, MaxUint256)
          await token0.approve(aggreswap.address, MaxUint256)
        })

        it('happy path :: Aggreswap', async () => {
          const WETHPairToken0 = await WETHPair.token0()

          const data = defaultAbiCoder.encode(
            ['uint256', 'address[]', 'address', 'uint256'],
            [0, [WETH.address, WETHPartner.address], wallet.address, MaxUint256]
          )

          await expect(
            aggreswap.swap(AddressZero, 0, router.address, '0x7ff36ab5', data, {
              ...overrides,
              value: swapAmount
            })
          )
      ...
      describe('swapExactTokensForETH :: Aggreswap', () => {
        const WETHPartnerAmount = expandTo18Decimals(5)
        const ETHAmount = expandTo18Decimals(10)
        const swapAmount = expandTo18Decimals(1)
        const expectedOutputAmount = bigNumberify('1662497915624478906')
        ...
        it('happy path :: Aggreswap', async () => {
          await WETHPartner.approve(router.address, MaxUint256)
          await WETHPartner.approve(aggreswap.address, MaxUint256)
          const WETHPairToken0 = await WETHPair.token0()

          const data = defaultAbiCoder.encode(
            ["uint256", "uint256", "address[]", "address", "uint256"],
            [swapAmount, 0, [WETHPartner.address, WETH.address], wallet.address, MaxUint256]
          );

          await expect(
            aggreswap.swap(WETHPartner.address, swapAmount, router.address, '0x18cbafe5', data, {
              ...overrides
            })
          )
            .to.emit(WETHPartner, 'Transfer')
